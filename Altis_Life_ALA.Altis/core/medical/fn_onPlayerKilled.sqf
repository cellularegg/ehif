...
_unit spawn
{
	...
	waitUntil {_Timer ctrlSetText format["Du kannst respawnen in: %1",[(_maxTime - time),"MM:SS.MS"] call BIS_fnc_secondsToString]; 
	round(_maxTime - time) <= 0 || isNull _this || life_request_timer};
	
	if (life_request_timer) then {
	_maxTime = time + (life_respawn_timer * 60 * 10);
	waitUntil {_Timer ctrlSetText format["Du kannst respawnen in: %1",[(_maxTime - time),"MM:SS.MS"] call BIS_fnc_secondsToString]; 
