...
if(isPlayer _curTarget && _curTarget isKindOf "Man") then {
	if((_curTarget getVariable["restrained",false]) && !dialog && (playerSide == west || (license_civ_ucduty))) then {
		[_curTarget] call life_fnc_copInteractionMenu;
	};
	if((_curTarget getVariable["restrained",false]) && !dialog && {playerSide in [civilian,independent]} && (player distance _curTarget < 3)) then {
	...
	// for hunting -GT-
	_huntingAnimals = ["Hen_random_F","Cock_random_F","Goat_random_F","Sheep_random_F","Rabbit_F"];
	
	if((typeOf _curTarget) in _huntingAnimals && !alive _curTarget) then {
		private["_handle"];
		_handle = [_curTarget] spawn life_fnc_gutAnimal;
		waitUntil {scriptDone _handle};
	};
