...
if([false,_food,1] call life_fnc_handleInv) then {
	switch (_food) do
	{
		...
		case "peach": {_val = 10};
		case "leberk": {_val = 40};
		case "krapfen": {_val = 20};
		case "frankfurter": {_val = 40};
		case "krainer": {_val = 40};
		case "manner": {_val = 20};
	};
	...
	if(_sum > 100) then {
		_sum = 100; 
		player setFatigue 1; 
		hint "Du hast dich überessen und fühlst dich jetzt eine Zeit lang träge!"; 
		[] spawn {
			sleep 30; 
			player setFatigue 0;	//Rewritten by -GT-
		};
	};
	life_hunger = _sum;
};