...
if((_this select 3) in ["cop","cob"] && playerSide != west) exitWith {hint localize "STR_Shop_NotaCop"; closeDialog 0;};
if((_this select 3) in ["dive"] && !license_civ_dive) exitWith { hint localize "STR_Shop_NotaDive"; closeDialog 0;};
if((_this select 3) in ["cob"] && !license_cop_swat) exitWith {hint "Du bist nicht bei der Cobra!"; closeDialog 0;};
if((_this select 3) in ["med"] && playerSide != independent) exitWith {hint "Du bist kein Sanitäter!"; closeDialog 0;};
